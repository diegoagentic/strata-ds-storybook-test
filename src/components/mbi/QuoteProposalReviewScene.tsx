            ...
            <div className="grid grid-cols-4 gap-3">
                {([
                    ...
                    <div key={s.label} className="bg-card dark:bg-zinc-800 border border-border rounded-xl p-3">
                        ...
                        <div className={`text-sm font-bold mt-0.5 ${s.highlight ? 'text-status-success' : 'text-foreground'} ${s.mono ? 'font-mono text-xs' : ''}`}>
                            {s.value}
                        </div>
                    </div>
                ))}
                                ...
                                <div className={`px-3 py-2.5 flex items-center gap-3 text-xs ${isEditing ? 'bg-primary/5 dark:bg-primary/10' : ''}`}>
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            {v.contract && (
                                                <span className="inline-flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-status-success/10 text-status-success mt-0.5">
                                    ...
                                    <div className="flex items-center gap-3 shrink-0">
                                        <div className={`text-sm font-bold tabular-nums ${v.locked ? 'text-status-success' : 'text-foreground'}`}>
                                            {gpPctDisplay}% GP
                                        </div>
                                        ...
                                        {v.locked ? (
                                            <Lock className="h-3.5 w-3.5 text-status-success/60 shrink-0" />
                                        ) : isEditing ? (
                                        ...
                                        <div className="rounded-xl overflow-hidden border border-border">
                                            ...
                                            {isEditing ? (
                                                ...
                                                <div className="grid grid-cols-[1.2rem_1fr_1fr_1fr_1fr_2.5rem_2rem_1fr_3.2rem] gap-x-2 px-3 py-2 text-[10px] tabular-nums bg-card dark:bg-zinc-800">
                                                    ...
                                                    <span className="text-status-success font-semibold">${Number(displayVal(v.vendor,'gp',v.gp)).toLocaleString()}</span>
                                                    <span className="text-status-success font-semibold">{displayVal(v.vendor,'gpPct',v.gpPct)}%</span>
