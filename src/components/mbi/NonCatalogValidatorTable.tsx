            ...
            <div className="px-4 py-3 border-b border-border">
                <div className="flex items-center justify-between">
                    ...
                    <div className="text-right text-[10px]">
                        <div>
                            <span className="text-status-success font-bold tabular-nums">{exactCount} exact</span>
            ...
            <div className="divide-y divide-border">
                {MOCK_ITEMS.map(item => {
                    ...
                    const rowAccent = isResolved
                        ? 'bg-status-success/5 dark:bg-status-success/10 border-l-4 border-l-success/60'
                    ...
                    return (
                        <div key={item.id} className={`px-4 py-2 grid grid-cols-[1.5fr_0.8fr_0.5fr_0.8fr_0.8fr_0.6fr] gap-3 items-center text-xs transition-colors ${rowAccent}`}>
                            <div className="min-w-0">
                                ...
                                {isResolved && (
                                    <div className="flex items-center gap-1 mt-0.5">
                                        <FileText className="h-2.5 w-2.5 text-status-success" />
                                        <span className="text-[9px] text-status-success font-bold">Quote-resolved · $215/u confirmed</span>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className={`text-right tabular-nums font-bold ${isResolved ? 'text-status-success' : 'text-foreground'}`}>
                                ...
                                {item.priceBook > 0 ? (
                                    <>
                                        ...
                                        {deltaPct !== 0 && !isResolved && (
                                            <div className={`text-[9px] font-semibold ${deltaPct > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-status-success'}`}>
                                                {deltaPct > 0 ? '+' : ''}{deltaPct}%
                                            </div>
                                        )}
                                    </>
...
function MatchPill({ match, confidence, quoteResolved }: { match: 'exact' | 'close' | 'mismatch'; confidence: number; quoteResolved?: boolean }) {
    if (quoteResolved) return (
        <span className="inline-flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-status-success/10 text-status-success" title="Resolved via vendor quote">
            <FileText className="h-2.5 w-2.5" />
            Resolved
        </span>
    )
    if (match === 'exact') return (
        <span className="inline-flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-status-success/10 text-status-success" title={`${confidence}% confidence`}>
            <CheckCircle2 className="h-2.5 w-2.5" />
            Exact
        </span>
    )
    if (match === 'close') return (
        <span className="inline-flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-status-info/10 text-status-info" title={`${confidence}% confidence`}>
