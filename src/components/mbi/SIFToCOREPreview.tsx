        ...
        <div className="bg-card dark:bg-zinc-800 border border-border rounded-2xl overflow-hidden">
            <div className="px-4 py-3 border-b border-border flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {isRunning && (
                        <span className="flex items-center gap-1.5 text-[11px] font-semibold text-status-ai">
                ...
                <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr_auto_1fr] items-stretch gap-3">
                    ...
                    <div className="hidden md:flex items-center justify-center">
                        <div className="flex flex-col items-center gap-1">
                            <ArrowRight className={`h-4 w-4 transition-colors duration-300 ${phase !== 'idle' ? 'text-status-ai' : 'text-border'}`} />
                            <span className={`text-[9px] font-bold uppercase tracking-wider transition-colors duration-300 ${phase !== 'idle' ? 'text-status-ai' : 'text-border'}`}>Auto</span>
                        </div>
                    </div>

                    {/* Column 2: AI extraction */}
                    <div className={`border rounded-xl p-3 flex flex-col transition-colors duration-300 ${phase !== 'idle' ? 'bg-status-ai/10 dark:bg-status-ai/15 border-status-ai/30' : 'bg-muted/20 border-border opacity-50'}`}>
                        <div className="flex items-center gap-1.5 mb-2">
                            {isRunning
                                ? <Loader2 className="h-3.5 w-3.5 text-status-ai animate-spin" />
                                : <Sparkles className="h-3.5 w-3.5 text-status-ai" />
                            }
                            <span className="text-[10px] font-bold text-status-ai uppercase tracking-wider">AI extraction</span>
                        </div>
                        ...
                        <div className={`mt-2 pt-2 border-t border-status-ai/20 text-[10px] transition-opacity duration-300 ${isDone ? 'opacity-100' : 'opacity-0'}`}>
                            <div className="font-semibold text-foreground">Confidence</div>
                            <div className="text-sm font-bold text-status-ai tabular-nums">96%</div>
                        </div>
                    </div>
