...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from 'strata-design-system'
import {
    PauseCircle, DollarSign, Clock, User, FileText,
...
function ModalContent({ record, onClose, onRelease, onComment }: {
    ...
    return (
        <>
            <DialogHeader className="px-5 py-4 border-b border-border bg-muted/40 dark:bg-zinc-800 space-y-0">
                <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0 text-left">
                        ...
                        <DialogDescription className="text-[11px] text-muted-foreground truncate m-0">
                            {record.client}
                        </DialogDescription>
                    </div>
                </div>
                ...
                <div>
                    ...
                    {mode === 'choose' && (
                        <div className="space-y-2">
                            <button
                                onClick={onRelease}
                                className="w-full flex items-center gap-2.5 px-4 py-3 rounded-xl text-sm font-bold bg-status-success/10 text-status-success hover:bg-status-success/15 border border-status-success/25 transition-colors text-left"
                            >
