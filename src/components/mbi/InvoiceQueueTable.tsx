...
const COLUMN_META: Record<InvoiceStatus, { label: string; sub: string; tone: string; chip: string }> = {
    ...
    'in-progress': {
        ...
        tone: 'border-status-ai/40 bg-status-ai/5 dark:bg-status-ai/10',
        chip: 'bg-status-ai/15 text-status-ai',
    },
    'done': {
        label: 'Done',
        sub: 'Auto-posted to CORE',
        tone: 'border-status-success/40 bg-status-success/5 dark:bg-status-success/10',
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center gap-0.5 flex-wrap">
                ...
                <span className={`ml-auto text-[9px] font-bold tabular-nums ${invoice.ocrConfidence >= 95 ? 'text-status-success' : invoice.ocrConfidence >= 90 ? 'text-zinc-900 dark:text-primary' : 'text-amber-700 dark:text-amber-400'}`}>
