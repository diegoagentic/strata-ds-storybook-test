...
export default function ARAgingReviewScene({ onContinue }: ARAgingReviewSceneProps = {}) {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="bg-status-ai/5 dark:bg-status-ai/10 border border-status-ai/30 rounded-xl p-3">
                <div className="flex items-center gap-1.5 mb-2">
                    <Sparkles className="h-3.5 w-3.5 text-status-ai" />
                    <span className="text-[10px] font-bold text-status-ai uppercase tracking-wider">Strata AR signals · today</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2">
                    ...
                    />
                    <InsightChip
                        icon={<TrendingUp className="h-3 w-3 text-status-success" />}
                        text={`$${(committed / 1000).toFixed(0)}K committed to pay this week — cash flow on track`}
                        tone="green"
                    />
                    <InsightChip
                        icon={<Mail className="h-3 w-3 text-status-ai" />}
    ...
    const cls = tone === 'red'
        ...
        : tone === 'green'
            ...
            : tone === 'amber'
                ...
                : 'bg-status-ai/5 border-status-ai/20'
    return (
