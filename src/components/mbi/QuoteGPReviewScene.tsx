        ...
        <div className="space-y-4">
            ...
            {(phase === 'creating' || phase === 'created') && (
                <div className={`rounded-2xl border overflow-hidden animate-in fade-in duration-300 transition-colors ${
                    phase === 'created' ? 'bg-primary/5 dark:bg-primary/10 border-primary/30' : 'bg-status-ai/5 dark:bg-status-ai/10 border-status-ai/30'
                }`}>
...
function GPVendorRow({
    ...
    return (
        <div className={`px-4 py-2.5 grid grid-cols-[1fr_auto_auto_auto] gap-4 items-center text-xs ${
            vendor.locked ? 'bg-status-success/5 dark:bg-status-success/10' : ''
        }`}>
            ...
            <div className="w-28 text-center">
                {vendor.contract ? (
                    <span className="inline-flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-status-success/15 text-status-success">
                    ...
                    <div className="relative">
                        <input
                            ...
                            className={`w-20 text-xs font-bold text-foreground text-right bg-background border rounded-lg px-2 py-1.5 pr-6 focus:outline-none focus:ring-1 focus:ring-primary transition-colors ${
                                value && !isValid ? 'border-destructive' : isValid ? 'border-status-success/50' : 'border-border'
                            }`}
                        />
            ...
            <div className="w-6 flex justify-center">
                {vendor.locked
                    ? <Lock className="h-3 w-3 text-status-success/60" />
                    : value && isValid
...
function CoreQuoteStat({ label, value, mono, highlight }: { label: string; value: string; mono?: boolean; highlight?: boolean }) {
    return (
        <div className="bg-card dark:bg-zinc-800 border border-border rounded-lg p-2.5">
            ...
            <div className={`text-sm font-bold mt-0.5 ${highlight ? 'text-status-success' : 'text-foreground'} ${mono ? 'font-mono text-xs' : ''}`}>
                {value}
            </div>
        </div>
    )
