...
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from 'strata-design-system'
import { ArrowRight, Database, Bot, MessageSquare, FileCode2, BookOpen } from 'lucide-react'
...
const TYPE_STYLE: Record<SourceType, { bg: string; text: string; dot: string }> = {
    ...
    ai:            { bg: 'bg-status-ai/10',          text: 'text-status-ai',                             dot: 'bg-status-ai' },
    external:      { bg: 'bg-amber-500/10',   text: 'text-amber-700 dark:text-amber-400', dot: 'bg-amber-500' },
    communication: { bg: 'bg-violet-500/10',  text: 'text-violet-700 dark:text-violet-400', dot: 'bg-violet-500' },
        ...
        <TooltipProvider delayDuration={150}>
            <Tooltip>
                ...
                <TooltipContent side="top" sideOffset={6} className="max-w-[240px]">
                    <div className="font-bold mb-0.5">{source.name}</div>
                    <div>{source.note}</div>
                </TooltipContent>
            </Tooltip>
        </TooltipProvider>
